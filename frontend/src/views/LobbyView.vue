  ...
  <div class="lobby-container">
    ...
    <header class="lobby-header">
      <div class="header-left">
        ...
        <span class="online-badge"> <span class="dot"></span> {{ onlineCount }} 人在线 </span>
      </div>
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item disabled
                ><span>🏆 积分：{{ userStore.userInfo.score }}</span></el-dropdown-item
              >
              <el-dropdown-item disabled
                ><span>⭐ 等级：Lv.{{ userStore.userInfo.level }}</span></el-dropdown-item
              >
      ...
      <aside class="lobby-sidebar">
        ...
        <div class="sidebar-section">
          ...
          <div class="quick-links">
            <div class="quick-link" @click="handleLink('history')">
              <el-icon><Clock /></el-icon> 对战记录
            </div>
            <div class="quick-link" @click="handleLink('rank')">
              <el-icon><Trophy /></el-icon> 排行榜
            </div>
            <div class="quick-link" @click="handleLink('guide')">
        ...
        <div class="room-grid" v-loading="isLoading">
          <template v-if="filteredRooms.length">
            <div
              ...
              :class="[
                'room-card',
                { 'is-full': room.playerCount >= room.maxPlayers || room.status === 'full' },
              ]"
              @click="
                room.playerCount >= room.maxPlayers || room.status === 'full'
                  ? null
                  : handleJoinRoom(room)
              "
            >
              <div class="room-card-header">
                ...
                <div class="room-tags">
                  <el-tag
                    :type="room.gameMode === 'single' ? 'success' : 'primary'"
                    size="small"
                    effect="plain"
                  >
              ...
              <div class="room-card-body">
                ...
                <div class="room-players">
                  ...
                  />
                  <span v-if="room.playerCount > 5" class="more-players"
                    >+{{ room.playerCount - 5 }}</span
                  >
                </div>
              </div>
              <el-progress
                :percentage="(room.playerCount / room.maxPlayers) * 100"
                :stroke-width="3"
                :show-text="false"
...
function handleJoinRoom(room) {
  ...
  if (room.playerCount >= room.maxPlayers || room.status === 'full') {
    ElMessage.warning('房间已满，无法加入')
    return
  }
  if (room.status === 'playing') {
    ElMessage.warning('游戏已开始，无法加入')
    return
  }
  if (room.status === 'finished') {
    ElMessage.warning('游戏已结束，无法加入')
    return
  }
  // 只有等待中的房间可以加入
  if (room.status !== 'waiting') {
    ElMessage.warning('该房间不可加入')
    return
  }
  if (room.hasPassword) {
...
async function doJoinRoom(room) {
  try {
    ...
    const recents = recentRooms.value.filter(function (r) {
      return r.id !== room.id
    })
