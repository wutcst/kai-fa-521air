...
/**
 * 集成测试基类 - 连接 Docker MySQL（3307）和 Docker Redis（6379） - 使用 RANDOM_PORT 启动完整 Spring 上下文 -
 * 每个测试类运行后清理测试数据
 *
 * <p>前置条件：docker-compose 已启动（MySQL + Redis）
 */
...
public abstract class BaseIntegrationTest {
    ...
    @Autowired protected SysUserRepository userRepository;

    @Autowired protected GameRepository gameRepository;

    @Autowired protected GamePlayerResultRepository playerResultRepository;

    @Autowired protected RoomRepository roomRepository;

    @Autowired protected RoomPlayerRepository roomPlayerRepository;
    ...
    /**
     * POST 请求辅助方法：发送 JSON body，返回 ResponseEntity 使用 HttpEntity（而非 RequestEntity），确保 4xx 响应 body
     * 正常返回
     */
    ...
    protected <T> ResponseEntity<T> getWithToken(String path, String token, Class<T> responseType) {
        RequestEntity<Void> request =
                RequestEntity.get(URI.create(baseUrl() + path))
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .build();
        return restTemplate.exchange(request, responseType);
    }

    /** 每个测试方法后清理所有测试数据 注意：外键约束要求按特定顺序删除 */
    @AfterEach
