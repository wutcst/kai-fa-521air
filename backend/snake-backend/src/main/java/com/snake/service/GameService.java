...
/** 游戏结果持久化服务 游戏结束时保存游戏记录和玩家成绩，并提供查询 API */
@Service
public class GameService {
    ...
    public GameEntity saveGameResult(Room room, GameResult result) {
        ...
        log.info(
                "Game saved: id={}, room={}, mode={}, duration={}s, players={}",
                gameId,
                room.getId(),
                game.getGameMode(),
                game.getDuration(),
    ...
    public Map<String, Object> getGameDetail(Long gameId) {
        ...
        List<GamePlayerResult> results =
                playerResultRepository.findByGameIdOrderByScoreDesc(gameId);
                                ...
                                r -> {
                                    ...
                                    if (!r.isBot()) {
                                        try {
                                            nickname =
                                                    userRepository
                                                            .findById(Long.parseLong(r.getUserId()))
    ...
    public Map<String, Object> getPlayerHistory(String userId, int page, int size) {
        ...
        List<Long> gameIds =
                playerResultRepository.findByUserId(userId).stream()
                        ...
                        .map(
                                g -> {
                                    List<GamePlayerResult> playerResults =
                                            playerResultRepository.findByGameIdAndUserId(
                                                    g.getId(), userId);
                                    GamePlayerResult myResult =
                                            playerResults.isEmpty() ? null : playerResults.get(0);
    ...
    public Map<String, Object> getPlayerStats(String userId) {
        ...
        double avgRank =
                totalGames > 0
                        ? (double) allResults.stream().mapToInt(GamePlayerResult::getRank).sum()
                                / totalGames
                        : 0;
